"""
tests/test_storage_and_images.py — Object storage (MinIO/S3) roundtrip.

These tests verify the full image lifecycle:
  1. POST /products/{id}/image      → upload bytes to MinIO
  2. GET  /products/{id}/image-url  → returns a pre-signed download URL
  3. HTTP GET on the pre-signed URL → returns the same bytes we uploaded

Plus a 404 case for products that have no image yet.

Why use raw httpx for step 3?
  The pre-signed URL points at MinIO directly (http://localhost:9000/...),
  not at the FastAPI app. We need a real network client, not the in-process
  ASGI transport, to fetch it.
"""
...
# A 1×1 transparent PNG — small enough to embed inline. MinIO doesn't care
# about format; this is just convenient real-looking image bytes.
...
async def test_image_upload_and_presigned_download_roundtrip(client: AsyncClient):
    ...
    # ---- Step 2: ask the API for a pre-signed download URL ----
    url_resp = await client.get(f"/products/{product_id}/image-url")
    assert url_resp.status_code == 200
    presigned = url_resp.json()["url"]

    # The URL targets MinIO directly, not the FastAPI app.
    # MinIO's default signing is SigV2 (`Signature=`), AWS S3 uses SigV4
    # (`X-Amz-Signature=`) — accept either so this test works against both.
    assert "Signature=" in presigned or "X-Amz-Signature=" in presigned
